        ...
        <div className="fixed inset-0 z-[150] bg-[#0B0E11] overflow-hidden flex flex-col font-sans">
            ...
            {viewingOrder && (
                <Modal isOpen={true} onClose={() => setViewingOrder(null)} maxWidth="max-w-4xl">
                    <div className="bg-[#1E2329] border border-[#2B3139] p-8 space-y-8 animate-in zoom-in duration-200 rounded-2xl">
                        <div className="flex justify-between items-start border-b border-[#2B3139] pb-6">
                            <div>
                                <h3 className="text-2xl font-bold text-[#EAECEF] tracking-tight">Order Details</h3>
                                <p className="text-sm text-[#848E9C] mt-1 font-mono">ID: {viewingOrder['Order ID']}</p>
                            </div>
                            <button onClick={() => setViewingOrder(null)} className="text-[#848E9C] hover:text-white transition-colors">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                            ...
                            <div className="space-y-8">
                                ...
                                <div>
                                    <label className="text-xs font-bold text-[#848E9C] uppercase tracking-wider mb-2 block">Delivery Address</label>
                                    <p className="text-sm text-[#EAECEF] leading-relaxed font-medium">{viewingOrder.Location}</p>
                                    {viewingOrder['Address Details'] && <p className="text-xs text-[#848E9C] italic mt-1.5">{viewingOrder['Address Details']}</p>}
                                    {viewingOrder.Note && (
                                        <div className="mt-4 p-4 bg-[#FCD535]/5 border border-[#FCD535]/20 rounded-xl">
                                            <p className="text-xs font-bold text-[#FCD535] uppercase mb-1.5">Special Instruction</p>
                                            <p className="text-sm text-[#EAECEF] leading-relaxed italic">"{viewingOrder.Note}"</p>
                                        </div>
                                    )}
                                </div>

                                <div>
                                    <label className="text-xs font-bold text-[#848E9C] uppercase tracking-wider mb-3 block">Payment Status</label>
                                    <div className="bg-[#0B0E11] p-4 rounded-xl border border-[#2B3139]">
                                        <div className="flex justify-between items-center mb-3">
                                            <span className="text-xs text-[#848E9C] font-bold">Total Amount</span>
                                            <span className="text-xl font-mono font-bold text-[#0ECB81]">${viewingOrder['Grand Total']?.toFixed(2) || '0.00'}</span>
                                        </div>
                                        <div className="flex items-center gap-3 pt-3 border-t border-[#2B3139]">
                                            {appData.bankAccounts?.find((b: any) => b.BankName === viewingOrder['Payment Info'])?.LogoURL ? (
                                                ...
                                                <div className="w-8 h-8 bg-[#2B3139] rounded flex items-center justify-center text-lg">
                                                    {viewingOrder['Payment Info'] === 'COD' || viewingOrder['Payment Status']?.includes('Unpaid') ? '💵' : '💳'}
                                                </div>
                                            )}
                                            <div>
                                                <p className="text-sm font-bold text-[#EAECEF] leading-tight">{viewingOrder['Payment Info'] || 'Unspecified'}</p>
                                                <span className="text-[10px] font-bold uppercase text-[#848E9C] tracking-tight">{viewingOrder['Payment Status']}</span>
                                            </div>
                                        </div>
                                    </div>

                                    {(viewingOrder['Delivery Unpaid'] || 0) > 0 && (
                                        <div className="mt-4 text-center bg-red-500/10 border border-red-500/20 p-3 rounded-xl">
                                            <p className="text-sm text-red-400 font-bold tracking-tight">Collect Cash: ${viewingOrder['Delivery Unpaid']?.toFixed(2)}</p>
                                        </div>
                                    )}
                                </div>
                            </div>

                            {/* Column 2: Fulfillment & Photos */}
                            <div className="space-y-8">
                                <div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex items-center gap-3 bg-[#0B0E11] p-3 rounded-xl border border-[#2B3139]">
                                            {appData.shippingMethods?.find((m: any) => m.MethodName === viewingOrder['Internal Shipping Method'])?.LogoURL ? (
                                                ...
                                                <div className="w-8 h-8 bg-[#2B3139] rounded flex items-center justify-center text-[10px] font-bold text-[#848E9C]">Ship</div>
                                            )}
                                            <div>
                                                <p className="text-sm font-bold text-[#EAECEF] leading-tight">{viewingOrder['Internal Shipping Method'] || 'TBD'}</p>
                                        ...
                                        <div className="flex items-center gap-3 bg-[#0B0E11] p-3 rounded-xl border border-[#2B3139]">
                                            {appData.pages?.find((p: any) => p.PageName === viewingOrder.Page)?.PageLogoURL ? (
                                                ...
                                                <div className="w-8 h-8 bg-[#2B3139] rounded-full flex items-center justify-center text-[10px] font-bold text-[#848E9C]">Src</div>
                                            )}
                                            <div>
                                                <p className="text-sm font-bold text-[#EAECEF] leading-tight">{viewingOrder.Page || 'Direct'}</p>
                                                <span className="text-[10px] text-[#848E9C] font-medium uppercase tracking-tighter">Source Page</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    <label className="text-xs font-bold text-[#848E9C] uppercase tracking-wider mb-3 block">Packaging Evidence</label>
                                    {getOptimisticPackagePhoto(viewingOrder['Order ID'], viewingOrder['Package Photo']) ? (
                                        <a href={getOptimisticPackagePhoto(viewingOrder['Order ID'], viewingOrder['Package Photo'])} target="_blank" rel="noreferrer" className="block relative group overflow-hidden rounded-2xl border border-[#2B3139]">
                                            <img src={getOptimisticPackagePhoto(viewingOrder['Order ID'], viewingOrder['Package Photo'])} alt="Package" className="w-full h-40 object-cover transition-transform duration-500 group-hover:scale-105" />
                                            <div className="absolute inset-0 bg-[#0B0E11]/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity duration-300">
                                                <span className="text-xs font-bold text-[#FCD535] uppercase tracking-widest bg-[#1E2329] px-4 py-2 rounded-lg border border-[#FCD535]/30">View Full Image</span>
                                            </div>
                                        </a>
                                    ) : (
                                        <div className="w-full h-40 bg-[#0B0E11] border border-dashed border-[#2B3139] rounded-2xl flex flex-col items-center justify-center text-[#848E9C] gap-2">
                                            <svg className="w-8 h-8 opacity-20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                                            <span className="text-xs font-medium uppercase tracking-widest opacity-40">No photo available</span>
                                        </div>
                                    )}
                                </div>
                            </div>

                            {/* Column 3: Items & Timeline */}
                            <div className="space-y-8 flex flex-col h-full">
                                <div className="bg-[#0B0E11] p-5 rounded-2xl border border-[#2B3139] flex-grow flex flex-col min-h-0">
                                    <div className="flex justify-between items-center mb-4 shrink-0">
                                        <label className="text-xs font-bold text-[#848E9C] uppercase tracking-wider">Item Allocation</label>
                                        <span className="text-xs font-bold text-[#FCD535] bg-[#FCD535]/10 px-2.5 py-1 rounded-lg border border-[#FCD535]/20">
                                            {viewingOrder.Products?.reduce((acc, p) => acc + p.quantity, 0) || 0} Total
                                        </span>
                                    </div>
                                    <div className="space-y-4 overflow-y-auto custom-scrollbar pr-2 flex-grow max-h-[280px]">
                                        {viewingOrder.Products?.map((p, i) => (
                                            <div key={i} className="flex gap-4 items-center border-b border-[#2B3139]/50 pb-4 last:border-0 last:pb-0">
                                                <div className="w-14 h-14 rounded-xl overflow-hidden bg-[#1E2329] border border-[#2B3139] shrink-0">
                                                    {p.image ? (
                                                ...
                                                <div className="flex-grow min-w-0">
                                                    <p className="text-sm font-bold text-[#EAECEF] leading-tight break-words line-clamp-2">{p.name}</p>
                                                    {p.colorInfo && <p className="text-xs text-[#848E9C] mt-1 font-medium italic">[{p.colorInfo}]</p>}
                                                </div>
                                                <div className="shrink-0 text-right">
                                                    <span className="text-base font-bold text-white bg-[#2B3139] px-2.5 py-1 rounded-lg border border-[#3B424A]">x{p.quantity}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div className="bg-[#1E2329] border border-[#2B3139] p-4 rounded-xl space-y-2 shrink-0">
                                    <div className="flex justify-between items-center text-xs font-medium">
                                        <span className="text-[#848E9C]">Order Timestamp:</span>
                                        <span className="text-[#EAECEF] font-mono">{viewingOrder.Timestamp}</span>
                                    </div>
                                    <div className="flex justify-between items-center text-xs font-medium pt-2 border-t border-[#2B3139]/50">
                                        <span className="text-[#848E9C]">Team Node:</span>
                                        <span className="text-[#EAECEF] font-bold uppercase tracking-tight">{viewingOrder.Team}</span>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div className="pt-6 flex justify-end border-t border-[#2B3139]">
                            <button onClick={() => setViewingOrder(null)} className="px-10 py-4 bg-[#FCD535] hover:bg-[#FCD535]/90 text-[#0B0E11] font-bold rounded-xl text-base transition-all active:scale-95 shadow-xl shadow-[#FCD535]/10">
                                Close Order Details
                            </button>
                        </div>
                    </div>
