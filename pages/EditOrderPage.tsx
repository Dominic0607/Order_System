...
const EditOrderPage: React.FC<EditOrderPageProps> = ({ order, onSaveSuccess, onCancel }) => {
    const { appData, currentUser, previewImage, refreshData, advancedSettings, hasPermission } = useContext(AppContext);
    ...
    useEffect(() => {
        if (currentUser) {
            ...
            if (!currentUser.IsSystemAdmin && !hasPermission('edit_order_global')) {
                // 1. Team Check
    ...
    const handleConfirmReturnPhoto = async (photo: string) => {
        ...
        try {
            ...
            const response = await fetch(`${WEB_APP_URL}/api/upload-image`, {
                ...
                body: JSON.stringify({
                    fileData: photo,
                    fileName: `return_${formData['Order ID']}_${Date.now()}.webp`,
                    mimeType: "image/webp",
                    orderId: formData['Order ID'],
                    team: formData.Team,
            ...
            await logUserActivity(currentUser?.UserName || 'Unknown', 'RETURN_ORDER', `Returned Order #${formData['Order ID']} (Backend Upload)`);
    ...
    return (
        ...
        <div className={`w-full h-full flex flex-col animate-fade-in overflow-hidden ${isLightMode ? 'bg-[#f8fafc]' : 'bg-[#0B0E11]'}`}>
            {/* Scanner Modal */}
            {isScannerVisible && (
                <BarcodeScannerModal 
            ...
            <div className={`flex-shrink-0 border-b px-3 sm:px-6 py-2 lg:py-2.5 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3 lg:gap-4 z-[60] shadow-sm relative ${isLightMode ? 'bg-white border-slate-200 text-slate-800' : 'bg-[#1E2329] border-[#2B3139] text-[#EAECEF]'}`}>
                ...
                <div className="hidden lg:flex gap-2 lg:w-auto mt-2 lg:mt-0">
                    ...
                    <button onClick={handleSubmit} disabled={loading} className={`flex-[2] lg:flex-none px-5 py-1.5 text-xs font-bold rounded-lg active:scale-[0.98] transition-all flex items-center justify-center gap-1.5 uppercase tracking-wider h-8 lg:h-9 ${isLightMode ? 'bg-blue-600 hover:bg-blue-500 text-white shadow-sm shadow-blue-500/10' : 'bg-[#FCD535] hover:bg-[#F0B90B] text-[#181A20] shadow-sm shadow-[#FCD535]/10'}`}>
                        ...
                        {!loading && <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                <div className="p-3 lg:p-4 flex flex-col gap-3 lg:gap-4 min-h-full max-w-[1600px] mx-auto w-full">
                    ...
                    <div className={`sticky bottom-0 z-30 pt-2 -mx-3 lg:-mx-4 px-3 lg:px-4 ${isLightMode ? 'bg-[#f8fafc]' : 'bg-[#0B0E11]'}`}>
                        <EditOrderSummary 
                                ...
                                <div className="bg-[#1E2329] border border-[#2B3139] overflow-hidden rounded-[3rem] border-white/10 p-6 space-y-6 animate-fade-in">
                                    ...
                                    {isSubmittingReturn ? (
                                        <div className="flex flex-col items-center justify-center space-y-4 py-12">
                                            <Spinner size="lg" />
                                            <p className="text-sm font-bold text-[#EAECEF]">កំពុងរក្សាទុក... (Saving...)</p>
                                        </div>
                                    ) : (
                                        <CameraCapture 
                                            onCapture={handleConfirmReturnPhoto}
