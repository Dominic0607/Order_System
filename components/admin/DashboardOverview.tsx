    ...
    return (
        <div className="space-y-6 animate-fade-in pb-10">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gray-800/10 p-5 rounded-[2rem] border border-white/5 backdrop-blur-md">
                <div>
                    <h2 className="text-xl lg:text-2xl font-black text-white leading-tight">សួស្តី, {currentUser?.FullName} 👋</h2>
                ...
                <div className="flex bg-black/40 p-1 rounded-xl border border-white/5">
                    {(['today', 'this_month', 'this_year'] as const).map(p => (
                        <button key={p} onClick={() => setRevenueBreakdownPeriod(p)} className={`px-4 py-1.5 text-[11px] font-black uppercase rounded-lg transition-all ${revenueBreakdownPeriod === p ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-500 hover:text-white'}`}>
