...
const ReportsView: React.FC<ReportsViewProps> = ({ orders, reportType }) => {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            const dataToAnalyze = reportType === 'shipping' ? shippingStats : stats;
                                    ...
                                    <tbody className="divide-y divide-gray-800">
                                        {shippingStats.drivers.map((d, i) => (
                                            <tr key={i} className="hover:bg-white/5">
                                                ...
                                                <td className="px-4 py-3 text-center font-black text-blue-400">{d.orders}</td>
                                                <td className="px-4 py-3 text-right font-black text-white">${d.cost.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                    <tfoot className="bg-white/5 border-t-2 border-white/10">
