                                    ...
                                    <div className={`p-2 border-t border-[#2B3139]/40 flex flex-col gap-2 bg-[#0B0E11]/20`}>
                                        {!isCancelled && activeTab === 'Ready to Ship' ? (
                                            <div className="space-y-2">
                                                ...
                                                {/* Photo Action */}
                                                {/* Show sent-status + delete if photo was already sent (regardless of group config) */}
                                                {(!!(order['Delivery Telegram Message ID'] || (order as any)['Delivery Telegram Message ID']) || localSentIds.has(order['Order ID']) || sessionStorage.getItem(`telegram_sent_${order['Order ID']}`) === 'true') ? (
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {/* Only show delete button to authorized users */}
                                                        {getCanSendToDriver(order) && (
                                                            <button
                                                                ...
                                                                className="w-10 h-10 flex items-center justify-center bg-red-500/10 text-red-500 border border-red-500/20 active:scale-95 rounded-xl transition-all disabled:opacity-50"
                                                            >
                                                    ...
                                                    /* Show send button only if delivery group has a Telegram group configured */
                                             ...
                                             <div className="flex flex-col gap-2 w-full">
                                                {/* If Shipped, also show Telegram Photo Action so they can see sent status and delete */}
                                                {activeTab === 'Shipped' && (
                                                              ...
                                                              <div className="flex items-center gap-2">
                                                                  {order['Return Photo'] ? (
                                                                      <button
                                                                          ...
                                                                          className="w-10 h-10 flex-shrink-0 rounded-xl overflow-hidden border-2 border-purple-500/40 hover:border-purple-500 transition-all relative group"
                                                                          title="មើលរូបភាព Return"
                                                                      >
                                                                          <img src={convertGoogleDriveUrl(order['Return Photo']!)} className="w-full h-full object-cover" alt="Return" />
                                                                      </button>
                                                                  ) : (
                                                                      <div className="w-10 h-10 flex-shrink-0 rounded-xl border border-purple-500/20 bg-purple-500/5 flex items-center justify-center">
                    ...
                    <div className={`flex items-center justify-between pt-4 pb-8 border-t ${B_BORDER} mt-4`}>
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                className={`px-4 py-1.5 border ${B_BORDER} rounded-sm text-xs font-bold uppercase transition-colors select-none ${isPageLoading || currentPage <= 1 ? 'opacity-30 cursor-not-allowed text-[#848E9C]' : `${B_BG_PANEL} ${B_TEXT_PRIMARY} active:border-[#FCD535]/50`}`}
                            >
                            ...
                            <button
                                ...
                                className={`px-4 py-1.5 border ${B_BORDER} rounded-sm text-xs font-bold uppercase transition-colors select-none ${isPageLoading || currentPage >= totalPages ? 'opacity-30 cursor-not-allowed text-[#848E9C]' : `${B_BG_PANEL} ${B_TEXT_PRIMARY} active:border-[#FCD535]/50`}`}
                            >
