                            ...
                            <div className={`px-3 py-1.5 border-b ${B_BORDER} flex justify-between items-center ${activeTab === 'Ready to Ship' ? 'pt-10' : ''}`}>
                                                ...
                                                <div key={order['Order ID']} className={`${B_BG_PANEL} border ${B_BORDER} ${B_BG_HOVER} transition-colors grid grid-cols-12 items-center gap-2 p-2 relative group cursor-pointer ${isCancelled ? 'bg-red-950/20 border-red-500/30' : isReturned ? 'bg-purple-950/20 border-purple-500/30' : ''}`} onClick={() => onView(order)}>
                                                    ...
                                                    <div className="col-span-2 min-w-0">
                                                        ...
                                                        {order.Note && (
                                                            <p className="text-[10px] text-[#FCD535] font-black italic truncate mt-1" title={order.Note}>Note: {order.Note}</p>
                                                        )}
                                                    ...
                                                    <div className="col-span-3 flex justify-end items-center gap-2">
                                                        ...
                                                        {activeTab === 'Cancelled' && order['Return Received By'] && (
                                                            <span className="text-[9px] font-black text-[#FCD535] uppercase px-2.5 py-1 bg-[#FCD535]/10 border border-[#FCD535]/30 rounded-sm truncate max-w-[150px] shadow-[0_0_10px_rgba(252,213,53,0.05)]" title={order['Return Received By']}>
                                                                Confirm by {order['Return Received By']}
                                                            </span>
                                                        )}
                                                        {!isCancelled && (
